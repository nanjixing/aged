...
<%--                    select的作用是点击那个变红--%>
    ...
    <span class="left_yh font24 width20 tcenter cursor onHover onorange">待支付（${fn:length(dzf)}）</span>
                    ...
                    <c:forEach items="${all}" var="data" varStatus="l">
                        <div class="width100 hidden_yh">
                            <div class="width100 hidden_yh fon20 c_66" style="background:#faf5f5;text-indent: 47px;height: 50px;line-height: 50px;border-bottom: 1px solid #ddd;">
                                ...
                                <c:if test="${data.status==-1}">待付款</c:if>
...
<%--                                        根据item_order中的status判断去付款--%>
                                        <c:if test="${data.status==-1}">
                                            <a href="${ctx}/pay/toPay?id=${data.pay.id}" class="onfff block_yh tcenter font16 onHoverr" style="margin-top: 10px;padding-right: 6px;">
                                                去付款
                                            </a>
                                        </c:if>
                                    </div>
                            ...
                            <div style="width: 838px;padding-top:30px;padding-bottom: 30px;" class="hidden_yh center_yh tleft">
                                ...
                                <c:if test="${data.status==0}">
                                    <a href="${ctx}/pay/refund?id=${data.pay.id}" class="c_33 onHover font20 onorange right_yh" style="margin-top: 10px;padding-right: 6px;">
                                        退款
                                    </a>
                                </c:if>
                                <c:if test="${data.status==1}">
                ...
                <!--待支付-->
                <div class="allGoods width100 hidden_yh hhD" style="display: none;">
                    <c:forEach items="${dzf}" var="data" varStatus="l">
                        <div class="width100 hidden_yh">
                            <div class="width100 hidden_yh fon20 c_66" style="background:#faf5f5;text-indent: 47px;height: 50px;line-height: 50px;border-bottom: 1px solid #ddd;">
                                <fmt:formatDate value="${data.addTime}" type="both"/>&nbsp;&nbsp;订单号${data.code}&nbsp;&nbsp;
                                <c:if test="${data.status==0}">待发货</c:if>
...
<%--    控制div中的select标签来进行控制根据我们选中的来进行显示，然后将其他的div设置为隐藏--%>
    ...
    var refund_msg = '${refund_msg}';
    if( refund_msg== "success"){
        alert("退款成功,请在已取消订单中查看");
    }
</script>
